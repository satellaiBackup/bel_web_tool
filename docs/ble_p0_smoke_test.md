...
| 当前协议默认通知策略 | `frontend/src/views/ble/legacyNotificationBoundary.test.ts` | NUS/APP/DFU 先逐通道放行，TRANSPORT 最后探测；可选特征缺失不返回红色 400，GATT 会话失效立即中止 |
...
5. 选择目标设备并连接，确认一次用户操作只产生一次 `/connect`；记录连接耗时和 NUS/APP/DFU/TRANSPORT 依次订阅的结果。每个小包通道就绪后应立即可用，不等待可选 TRANSPORT 探测完成。
6. 确认独立 Nordic UART Service 的 Notify 特征 `6e400003-b5a3-f393-e0a9-e50e24dcca9e` 已请求 `/subscribe`。TRANSPORT 帧内保留的逻辑 CH=0 不影响独立 NUS Service。若 TRANSPORT 不存在，接口应以 `200 + unsupported` 返回，保持物理连接并显示 `connected_partial`，NUS/APP/DFU 写入不受阻；若出现 `BLE GATT 会话失效`，连接初始化必须立即失败。
